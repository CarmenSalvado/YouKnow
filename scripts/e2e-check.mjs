...
try {
  ...
  await eventually(`document.querySelectorAll('.metro-station').length > 12`, 'The starter route did not load.')
  ...
  await eventually(`document.querySelector('[aria-label="Learning title"]')?.value === 'Quantum Computing' && document.querySelectorAll('.metro-station').length > 12`, 'The Quantum route is not preloaded.')
  ...
  await run(`(() => { const input = document.querySelector('[aria-label="Learning title"]'); Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set.call(input, 'Urban Beekeeping'); input.dispatchEvent(new Event('input', { bubbles: true })); })()`)
  ...
  if (await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).title`) !== 'Urban Beekeeping') throw new Error('The arbitrary topic was not used.')
  const stationCount = await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).concepts.length`)
  if (stationCount < 13 || stationCount > 25) throw new Error('The custom plan did not start with enough stations.')
  if (!await run(`(() => { const plan = JSON.parse(localStorage.getItem('metro-plan:quantum')); return plan.concepts.find(concept => concept.id === plan.goal_concept_id)?.name === 'Urban Beekeeping' && document.querySelector('.map-route-position .is-goal')?.textContent.includes('Urban Beekeeping') })()`)) throw new Error('The requested topic was not the end goal.')
  if (!await run(`(() => { const plan = JSON.parse(localStorage.getItem('metro-plan:quantum')); const assigned = plan.lines.flatMap(line => line.concept_ids); return plan.lines.length > 0 && assigned.length === plan.concepts.length && new Set(assigned).size === plan.concepts.length })()`)) throw new Error('The API did not organize every station into named lines.')
  ...
  if (!await run(`(() => { const plan = JSON.parse(localStorage.getItem('metro-plan:quantum')); const tracks = [...document.querySelectorAll('.generated-routes > path:not(.concept-edge)')]; return tracks.length === plan.lines.filter(line => line.concept_ids.length > 1).length && tracks.every(track => /[HVQ]/.test(track.getAttribute('d'))) })()`)) throw new Error('Generated concepts were not rendered as continuous metro tracks.')
  if (!await run(`(() => { const plan = JSON.parse(localStorage.getItem('metro-plan:quantum')); const goalLine = plan.lines.find(line => line.concept_ids.includes(plan.goal_concept_id)); return plan.edges.some(edge => edge.to === plan.goal_concept_id && (goalLine?.concept_ids.includes(edge.from) || document.querySelector('[data-edge-from="' + CSS.escape(edge.from) + '"][data-edge-to="' + CSS.escape(edge.to) + '"]'))) })()`)) throw new Error('The end goal is disconnected from the graph.')
  const prerequisiteTarget = await run(`(() => { const plan = JSON.parse(localStorage.getItem('metro-plan:quantum')); return plan.edges[plan.edges.length - 1]?.to })()`)
  ...
  const plannedMinutes = await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).statistics.total_minutes`)
  await clickText('Mark as known')
  if (!await run(`JSON.parse(localStorage.getItem('metro-known-concepts:quantum')).includes(${JSON.stringify(prerequisiteTarget)}) && JSON.parse(localStorage.getItem('metro-plan:quantum')).statistics.total_minutes === ${plannedMinutes}`)) throw new Error('Marking a known station changed the planned hours.')
  ...
  await run(`document.querySelectorAll('.generated-station')[1]?.dispatchEvent(new MouseEvent('click', { bubbles: true }))`)
  const chosenLesson = await run(`document.querySelector('.map-heading h1')?.textContent`)
  await clickText('Start lesson')
  ...
  if (await run(`JSON.parse(localStorage.getItem('metro-completed-sessions:quantum')).length`) !== 1) throw new Error('Lesson progress was not persisted.')
  ...
  if (!await run(`JSON.parse(localStorage.getItem('metro-notes:quantum'))[${JSON.stringify(chosenConceptId)}] === 'A note that belongs only to this node.' && JSON.parse(localStorage.getItem('metro-sources:quantum')).some(source => source.conceptId === ${JSON.stringify(chosenConceptId)} && source.label === 'https://example.com/node-source')`)) throw new Error('The node note and source were not persisted together.')
  ...
  await clickText('Settings')
  const completedPlanSize = await run(`(() => { const plan = JSON.parse(localStorage.getItem('metro-plan:quantum')); const completed = plan.schedule.map(session => session.date + ':' + session.concept_id); localStorage.setItem('metro-completed-sessions:quantum', JSON.stringify(completed)); location.reload(); return completed.length })()`)
  ...
  if (await run(`JSON.parse(localStorage.getItem('metro-completed-sessions:quantum')).length`) !== completedPlanSize) throw new Error('Review duplicated completed progress.')

  const conceptsBeforeExtension = await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).concepts.length`)
  const extensionTarget = await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).concepts.at(-1).id`)
  ...
  const conceptsAfterExtension = await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).concepts.length`)
  ...
  if (!await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).lines.some(line => line.id.startsWith('extension:') && document.querySelector('.legend')?.textContent.includes(line.name))`)) throw new Error('The API-named extension lines were not shown on the map.')
  if (!await run(`JSON.parse(localStorage.getItem('metro-plan:quantum')).edges.some(edge => edge.to === ${JSON.stringify(extensionTarget)} && edge.from.startsWith('extension:'))`)) throw new Error('The new prerequisite track was not connected to the selected station.')
  ...
  await eventually(`document.querySelector('.user-card')?.textContent.includes('Ada Lovelace') && document.querySelector('[aria-label="Learning title"]')?.value === '' && !!document.querySelector('.empty-map') && document.querySelectorAll('.metro-station').length === 0`, 'The new profile was not activated as an empty profile.')
  ...
  await eventually(`document.querySelector('.user-card')?.textContent.includes('Alex Morgan') && !JSON.parse(localStorage.getItem('metro-profiles')).some(profile => profile.name === 'Ada Lovelace')`, 'The profile was not deleted safely.')
