...
const stops = [
  { id: 'central', title: 'Learn anything. Without getting lost.', copy: 'YouKnow turns a huge topic into a clear route: what to learn, in what order, and how far to move each day.', color: '#3b82f6', position: [520, 110] },
  { id: 'entrada', title: 'Tell us where you want to go.', copy: 'Enter a topic, paste a source, or set a goal. You do not need to prepare a syllabus before you begin.', color: '#f04f3d', position: [330, 110] },
  { id: 'conexiones', title: 'Start with what you need to know.', copy: 'YouKnow finds the prerequisites and connects every concept. See the full journey before taking the first step.', color: '#b56cff', position: [220, 220] },
  { id: 'ritmo', title: 'A plan that actually fits your day.', copy: 'Set the time you have each day. Your route calculates when you will master the topic and recalculates as your pace changes.', color: '#ffb52e', position: [220, 480] },
  { id: 'progreso', title: 'Always know where you are.', copy: 'Move forward one station at a time. The line makes it clear what you have mastered and what comes next.', color: '#29c58a', position: [400, 660] },
  { id: 'destino', title: 'Your next line starts today.', copy: 'Choose a topic. YouKnow handles the order. You only need to reach the next stop.', color: '#ff5b94', position: [580, 660] },
] as const
          ...
          <section id={stop.id} data-index={index} data-active={index === active || undefined} className="landing-stop" key={stop.id}>
            <div className={`landing-copy ${index === active ? 'is-shown' : 'is-hiding'}`}>
              <h1 className="reveal-line">
                {index === 0 ? <><span>Learn anything.</span><span>Without getting lost.</span></> : stop.title}
              </h1>
